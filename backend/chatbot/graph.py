...
- Frontend: React upload screen (supporting PDFs and images), preset templates, extraction progress, results review, verify/edit mode, dashboard/history view, cloud integration with Google Drive and OneDrive.
- Backend: Flask APIs for document/image extraction, extraction history, extraction status, templates, chat, and cloud file extraction.
- Main user flow: upload PDF or image → choose preset or custom fields → POST /api/extract → show JSON results; dashboard/history metrics; cloud integration routes for file selection and extraction.
You have access to this PostgreSQL schema:
- extraction_records(id, template_name, timestamp, file_name, data_points JSON, results JSON)
